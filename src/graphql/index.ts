...
export type GetStaticUserQueryVariables = Exact<{ [key: string]: never }>;

export type GetStaticUserQuery = { __typename?: 'Query' } & {
  user?: Maybe<{ __typename?: 'User' } & Pick<User, 'id' | 'name' | 'email'>>;
};

export const GetStaticUserDocument = gql`
  query getStaticUser {
    user(id: 1) {
      id
      name
...
export function useGetStaticUserQuery(
  options: Omit<Urql.UseQueryArgs<GetStaticUserQueryVariables>, 'query'> = {}
) {
  return Urql.useQuery<GetStaticUserQuery>({
    query: GetStaticUserDocument,
    ...options,
  });
}
